...
async function deployAccount(accountName: string) {
    try {
        const res = execSync(`starknet deploy_account --account=${accountName}`);        
    } catch (error) {
        console.log(error)
    }
    const accounts = readFileSync(homedir() + '/.starknet_accounts/starknet_open_zeppelin_accounts.json', 'utf8');
